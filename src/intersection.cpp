...
namespace
{
std::optional<IntersectionResult> intersectSkewOrCrossing(const Vector3D &dirA, const Vector3D &dirB,
														  const Vector3D &startA, const Vector3D &startB, double eps)
{
	...
	const double determinant = dirA_len2 * dirB_len2 - dirA_dot_dirB * dirA_dot_dirB;
	...
	const double paramOnA_unclamped = (dirA_dot_dirB * dirB_dot_w - dirB_len2 * dirA_dot_w) / determinant;
	const double paramOnB_unclamped = (dirA_len2 * dirB_dot_w - dirA_dot_dirB * dirA_dot_w) / determinant;
	...
	{
		return IntersectionResult{Vector3D((pointOnA.x() + pointOnB.x()) * 0.5, (pointOnA.y() + pointOnB.y()) * 0.5,
...
std::optional<IntersectionResult> intersectColinear(const Vector3D &dirA, const Vector3D &startA, const Segment3D &B,
													double eps)
{
	const double dirA_len2 = dirA.dot(dirA);

	// If segment A is a point
	if (dirA_len2 <= eps)
	{
		const Vector3D pointA = startA;
		const Vector3D dirB = B.direction();
		const double dirB_len2 = dirB.dot(dirB);

		if (dirB_len2 <= eps)
		{
			// Both segments are points
			if ((pointA - B.start()).length() <= eps)
				return IntersectionResult{pointA};
			else
				return std::nullopt;
		}

		// Only A is a point – check if it lies on segment B
		const double t = dirB.dot(pointA - B.start()) / dirB_len2;
		if (t >= -eps && t <= 1.0 + eps)
			return IntersectionResult{pointA};
		else
			return std::nullopt;
	...
	auto project = [&](const Vector3D &point) -> double { return dirA.dot(point - startA) / dirA_len2; };
	...
	{
		// Overlap degenerates to a point
		const double mid = (overlap_start + overlap_end) * 0.5;
		return IntersectionResult{startA + dirA * mid};
	}
	// Return the overlapping segment
	return IntersectionResult{Segment3D(startA + dirA * overlap_start, startA + dirA * overlap_end)};
}

} // namespace

std::optional<IntersectionResult> intersect(const Segment3D &A, const Segment3D &B, double eps)
{
	const Vector3D dirA = A.direction();
	const Vector3D dirB = B.direction();
